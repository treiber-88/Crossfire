...
SendIndianInfantry = function()
	...
	local units = Reinforcements.Reinforce(india, Force1, Ambush1Path, DateTime.Seconds(ForceInterval))
	Utils.Do(units, function(unit)
		BindActorTriggers(unit)
	end)
	Ambush2Path = { Ambush3.Location, Rally2.Location }
	units = Reinforcements.Reinforce(india, Force2, Ambush2Path, DateTime.Seconds(ForceInterval))
	Utils.Do(units, function(unit)
		BindActorTriggers(unit)
	end)
	Ambush3Path = { Ambush5.Location, Rally3.Location }
	units = Reinforcements.Reinforce(india, Force3, Ambush3Path, DateTime.Seconds(ForceInterval))
	Utils.Do(units, function(unit)
		BindActorTriggers(unit)
	end)
	Trigger.AfterDelay(DateTime.Seconds(ForceDelay1),SendIndianInfantry )
end

SendIndianBuggies = function()
	buggyPath = { Ambush2.Location, Rally1.Location }
	local units = Reinforcements.Reinforce(india, Force4, buggyPath, DateTime.Seconds(ForceInterval))
	Utils.Do(units, function(unit)
		BindActorTriggers(unit)
	end)
	Trigger.AfterDelay(DateTime.Seconds(ForceDelay2),SendIndianBuggies )
end
...
SendIndianTanks = function()
	...
	Trigger.AfterDelay(DateTime.Seconds(ForceDelay3),SendIndianTanks )
end
...
WorldLoaded = function()
	...
	Trigger.AfterDelay(DateTime.Seconds(5), function() Actor.Create("camera", true, { Owner = player, Location = InsertionLZ.Location }) end)
	Trigger.AfterDelay(DateTime.Seconds(5), function() Media.PlaySpeechNotification(player, "PakMission01Briefing") end)
	Trigger.AfterDelay(DateTime.Seconds(10), SendJeeps)
	Trigger.AfterDelay(DateTime.Seconds(ForceDelay1),SendIndianInfantry )
	Trigger.AfterDelay(DateTime.Seconds(ForceDelay2),SendIndianBuggies )
	Trigger.AfterDelay(DateTime.Seconds(ForceDelay3),SendIndianTanks )
